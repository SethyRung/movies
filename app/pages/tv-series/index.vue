...
const sortOptions = [
  ...
  { label: "First Aired", value: "firstAiredYear" },
];
...
useSeoMeta({
  title: "Browse TV Series - Cine Max",
  description: "Explore our collection of TV series at Cine Max",
});
</script>
      ...
      <UContainer>
        <div class="pt-[calc(var(--ui-header-height)+2rem)] pb-8">
          <div class="max-w-7xl mx-auto">
            ...
            <div v-else-if="series.length === 0" class="text-center py-20">
              <div
                class="w-20 h-20 border border-stone-800 rotate-45 flex items-center justify-center mx-auto mb-6"
              >
                <UIcon name="i-lucide-tv" class="w-8 h-8 text-stone-600 -rotate-45" />
              </div>
              <p class="text-stone-400 text-lg mb-2">No TV series found</p>
            ...
            <template v-else>
              ...
              <div
                class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6"
              >
                <MediaPosterCard v-for="item in series" :key="item.id" :content="item" />
              </div>
